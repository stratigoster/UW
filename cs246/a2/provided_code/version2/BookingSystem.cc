...
void BookingSystem::listAllFreeRooms( Period *period ) {
    std::map< std::string, Building *>::iterator position = buildings.begin();
    std::cout << "The rooms free for the period " << period << " are:" 
              << std::endl;
    for ( ; position != buildings.end(); position++ ) {
        position->second->listAllFreeRooms( period );
    } // for
...
std::ostream & operator<<( std::ostream & output, const BookingSystem & b ) {
    std::map< std::string, Building *>::const_iterator pos;
    for ( pos = b.buildings.begin(); pos != b.buildings.end(); pos++ ) {
